...
+ **Tidyverse-style** functionality!
   ...
   - Exports [{dplyr}](https://dplyr.tidyverse.org)-like **`_data()`** 
   functions e.g., `arrange_data()` `filter_data()`, `select_data()`, 
   `mutate_data()`, `summarise_data()`, `bind_rows_data()`, `bind_cols_data()`,
   `full_join_data()`, `left_join_data()`, `right_join_data()`
+ Simple and effective **printing of data frames**
   - Provides **[{tibble}](https://tibble.tidyverse.org)-like** printing experience
+ Extremely **lightweight**!
   - Has **zero dependencies**-only requires base R
   - Compiles **quickly and easily**-ideal for new users, new/difficult machines, working remotely, etc. 
...
+ **`tbl_dat_frame()`: Create data frames using previously defined variables
